...
from model_specification import ModelSpecifications
...
class MBWrapper:
    ...
    def load_data(self, category):
        ...
        :return:
        '''
        df = pd.read_pickle("/beegfs/zhukova/mbg_jupyter/input/balanced-datasets/" +
                            str(self.category) + "-balanced_data.pkl")
        data = []
    ...
    def run_parallel(self, args: list):
        ...
        for arg in args:
            # with queue (queue,) +
            p = Process(target=training.training, args=arg)
            p.start()
            process.append(p)
        for i in process:
            i.join()
    ...
    def main(self):
        ...
        model, tokenizer, learning_rate = specs.modelspecifications(
            self.model_name, self.model_length)  # Get Model Specifications form ModelSpecifications.py
        ...
        try:
            for fold, (train_ids, val_ids) in enumerate(splits.split(np.arange(len(data)), [ele['dataset_id'] for ele in data])):
                start = time.time()
                score = training.main(fold, train_ids, val_ids, data, copy.deepcopy(
